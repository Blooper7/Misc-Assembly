...
section .text
    ...
    mov rbx, '/bin/sh'          ; Only works in NASM. For other assemblers, use 0x0068732f6e69622f ("/bin/sh" in hex)
                                ; //bin/sh is backwards because it will get popped in reverse order
                                ; it's //bin/sh instead of /bin/sh because it's safer to align to 8 bytes to avoid a null byte. 
                                ; // is harmless, and functionally identical to /
